...
Binorm_diagnostics_mixed <- function(a00, a01, a101, a111, a102, a112, pi,
  ...
  # Y0 ~ N(a_00 + a_01*X_0, \sigma_0)
  # Y1 ~ N(pi*(a_101 + a_111*X_1) + (1-pi)*(a_102 + a_112*X_1), \sigma_1)
...
Bigamma_diagnostics_linear_cov <- function(shape0, shape1, a00,a01,a10,a11, cov, 
                                           grid = seq(0,1,length=100)){
  # grid <- seq(0,1,length=100)
  scale0 = (a00 + a01* cov)
  scale1 = (a10 + a11* cov)
  
  ROC = 1-pgamma(qgamma(1-grid, shape = shape0, scale = scale0), shape = shape1, scale = scale1)
  AUC <- trap(ROC)
  # AUC <- 1-pf(scale0/scale1, df1 = 2*shape, df2 = 2*shape)
  out=list(ROC = ROC, AUC = AUC)
  return(out)
...
BN = function(seed, N=50, X0, X1, c = c(0,1)){
  ...
  # bn2 = Binorm_diagnostics_linear_cov(a01=b00,a11=b01,
  #                                     a02=b10,a12=b11,
  #                                     sigma0=sd0,sigma1=sd1,cov = c[3])
...
Skewed = function(seed, N=50, X0, X1, c = c(0,1)){
  ...
  set.seed(seed)
  # y0 = rgamma(N, shape = shape0, scale = exp(b00+b01*X0))
  # y1 = rgamma(N, shape = shape1, scale = exp(b10+b11*X1))
...
Mixed = function(seed, N=50, X0, X1, c = c(0,1)){
  ...
  # Y0 ~ N(a00 + a01*X0, sd0)
  # Y1 ~ N(pi*(a101 + a111*X1) + (1-pi)*(a102 + a112*X1), sd1)
  # pi*(a101 + a111*(-0.5)) + (1-pi)*(a102 + a112*(-0.5))
  ...
  bm1 = Binorm_diagnostics_mixed(a00, a01, a101, a111, a102, a112, pi,
                                 sigma0 = sd0,sigma1=sd1,cov = c[2])
  # bm2 = Binorm_diagnostics_mixed(a00, a01, a101, a111, a102, a112, pi,
  #                                sigma0 = sd0,sigma1=sd1,cov = c[3])
...
BiNormal_JAGS_fit <- function(y0, y1, X0, X1, burnin = 5000, numb.iter = 5000){
  ...
  # binorm_1 = coda.samples(binorm.model, variable.names=c("b00","b10","b01","b11","sigma0","sigma1"), 
  # numb.iter,thin=numb.iter/5000, progress.bar="none")
  
  #MCMCsummary(binorm_1,params=c("b00","b10","b01","b11","sigma0","sigma1"),n.eff = TRUE)
  #MCMCplot(binorm_1,params=c("b00","b10","b01","b11","sigma0","sigma1"))
  #MCMCtrace(binorm_1,params=c("b00","b10","b01","b11","sigma0","sigma1"))
...
BiNormal_reg_fit <- function(y0, y1, X0, X1, x, burnin = 5000, numb.iter = 5000){
  ...
  # bn3 = BN_accuracy_cutoff(b00=b00, b01=b01, b10=b10, b11=b11, 
  #                          sigma0=sigma0, sigma1=sigma1, x=x[3])
...
cutoff.func = function(seed, N = 100, data.gen = BN, x = c(0, 1),
  ...
  # seed = 2000; N = 100; data.gen = BN; x = c(0, 1)
  # burnin = 5000; numb.iter = 5000
  # seed = 123; N = 100; data.gen = Skewed; x = c(0, 1);
  # burnin = 5000; numb.iter = 5000
  ...
  PV.fit = freqPV_reg_fit(y0, y1, X0, X1, x)
